...
// File: contracts\ERC20Set.sol

contract ERC20BasicSet {
    using SafeMath for uint256;
...
contract ERC20Set is ERC20BasicSet, Ownable {
    ...
    constructor(string memory name, string memory symbol, uint256 finalsupply, address _WLAddress) public {
        _name = name;
        _symbol = symbol;
        _decimals = 18;
        _finalSupply = finalsupply;
        setWLAddress = _WLAddress;
    ...
    modifier holderUnderMax(address investor, uint256 amountToAdd) {
        uint256 holderBalanceToBe = balanceOf(investor) + amountToAdd;
        bool okToTransfer = holderBalanceToBe <= WLContract.getMaxWLAmount(investor) && WLContract.isWhitelisted(investor) ? true :
                          holderBalanceToBe <= WLContract.getWLThresholdBalance() ? true : false;
        require(okToTransfer, "Investor not allowed to perform operations!");
        _;
    }
    ...
    function tokenTransferForm(address vendor, address receiver, uint256 amount) public holderUnderMax(receiver, amount) {
        transferFrom(vendor, receiver, amount);
    }

    function tokenTransfer(address receiver, uint256 amount) public holderUnderMax(receiver, amount) {
